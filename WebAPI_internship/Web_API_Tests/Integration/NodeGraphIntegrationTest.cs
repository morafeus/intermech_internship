...
namespace Web_API_Tests.Integration
{
    public class NodeGraphIntegrationTest
    {
        ...
        [Test]
        public async Task ChangeNodesBad()
        {
            const string testName = "Alexey";
            const string testPassword = "Password123";
            ...
            var responsePutNodes = await _client.PutAsync($"https://localhost:7276/api/nodegraph/{nodeId}?name={nodeNameChange}", new StringContent(""));

            Assert.That(responsePutNodes.StatusCode, Is.EqualTo(HttpStatusCode.BadRequest));
        }
